...
<input type="hidden" id="orderType" name="orderType" value="${orderType}" />
    ...
    <div class="list mt_10">
        <ul class="message_list">
            ...
            <li class="flexLayout">
                ...
                <div class="list_right">
                	<input type="hidden" name="rentTime">
                    <span class="mr_15" id="rentTime" ></span></div>
            </li>
            <li class="flexLayout">
            ...
            <li class="flexLayout">
                ...
                <c:if test="${rzStatus_ == '已认证'  && sqStatus_ == '已授权'}">
                 <c:if test="${mySesame < credit }">
                	<div class="list_right text-danger"><span class="mr_15">-￥<span id="reliefDeposit">0</span></span></div>
                </c:if>
                <c:if test="${mySesame > credit }">
                	<c:if test="${credit1 > cashPledge_ }">
                	 	<div class="list_right text-danger"><span class="mr_15">-￥<span id="reliefDeposit">${cashPledge_}</span></span></div>
                	</c:if>
                	<c:if test="${credit1 < cashPledge_ }">
                	 	<div class="list_right text-danger"><span class="mr_15">-￥<span id="reliefDeposit">${credit1}</span></span></div>
                	</c:if>
                </c:if>
                 </c:if>
                 <c:if test="${rzStatus_ != '已认证'  || sqStatus_ != '已授权'}">
            ...
            <li class="flexLayout message">
                <div class="list_left1">留言</div>
                <div class="list_right1"><textarea type="text" name="remark" id="remark" placeholder="请输入您想备注的信息"/></textarea></div>
            </li>
        </ul>
    </div>
 ...
 <!--    <div class="foot_btn flexLayout" style="display:none"> -->
	...
	<%-- <c:forEach items="${coupons}" var="coupon">
				<a onclick="setCoupon('${coupon.items.Denomination_ }','${coupon.items.couponId_ }');">
					...
					<span >${coupon.items.Denomination_ }元优惠券</span>
					<span >有效期：
					<fmt:formatDate value="${coupon.items.enddate_ }" pattern="yyyy-MM-dd HH:mm:ss"/>
					-<fmt:formatDate value="${coupon.items.enddate1_ }" pattern="yyyy-MM-dd HH:mm:ss"/>
					</span>
				</a>
		</c:forEach>  --%>
...
function setCoupon(Denomination_,couponId_){
	...
	$("input[name='Denomination_']").val(Denomination_);
