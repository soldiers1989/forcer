...
<script type="text/javascript">
	function yanzheng(){
		var salePrice_ = $("#salePrice_").val();
		var saleMemPrice_ = $("#saleMemPrice_").val();
		var offerCount_ = $("#offerCount_").val();
		var offerPrice_ = $("#offerPrice_").val();
		var saleJiFen_ = $("#saleJiFen_").val();
		var proColor_ = $("#proColor_").val();
		var posTage_ = $("#posTage_").val();
		var  color = /^\u4e00-\u9fa5+$/;
		var  regRule = /\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDE4F]/g;
		var num = /^[0-9]*[1-9][0-9]*$/;
		var money = /^\d+(\.\d{1,2})?$/;
		...
		if(!money.test(salePrice_) || !money.test(saleMemPrice_) || !money.test(offerPrice_)){
			alert("价格格式有误");
			return false;
		}
		if(!num.test(offerCount_)){
			alert("商品优惠数量只可为正整数");
			return false;
		}
		if(!num.test(saleJiFen_)){
			alert("积分只可为正整数");
			return false;
		}
		if(!num.test(posTage_)){
			alert("快递费用只可为正整数");
			return false;
		}
...
<div class="mid-box fl" id="mid-box">
	<div class="top pl_20 pr_20 mb_20">
       <span class="min-col f16">添加售卖物品属性</span>
    </div>
	...
	<form action="FrmSale_proManage.insertSaleProAttr" method="post" id="proAttr" class="formb" onsubmit="return yanzheng()">
				...
				<div class="group-box" name="null" id="component1" visible="false">
					<label style="width:200px">物品原价：</label><input name="salePrice_" id="salePrice_">元
				</div>
				<div class="group-box" name="null" id="component1" visible="false">
					<label style="width:200px">物品会员价：</label><input name="saleMemPrice_" id="saleMemPrice_">元
				</div>
				<div class="group-box" name="null" id="component1" visible="false">
					<label style="width:200px">商品优惠价格：</label>
					<label>商品优惠数量：</label>
					<input name="offerCount_"  style="width:100px;" id="offerCount_">个
					<label>优惠价格：</label>
					<input name="offerPrice_" style="width:100px;" id="offerPrice_">/元
				</div>
				<div class="group-box" name="null" id="component1" visible="false">
					<label style="width:200px">物品条形码：</label>
					<input type="text" name="barCode_" >
				</div>
				<div class="group-box" name="null" id="component1" visible="false">
					<label style="width:200px">消费赠送积分：</label><input name="saleJiFen_" id="saleJiFen_">积分/件
				</div>
				<div class="group-box" name="null" id="component1" visible="false">
					<label style="width:200px">物品货号：</label><input name="proNumber_">
				</div>
				<div class="group-box" name="null" id="component1" visible="false">
					<label style="width:200px">物品颜色：</label><input name="proColor_" id="proColor_">
				</div>
				<div class="group-box" name="null" id="component1" visible="false">
					<label style="width:200px">物品同城快递邮费：</label><input name="posTage_" id="posTage_">元
				</div>
