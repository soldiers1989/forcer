 ...
 * 
 * @author xiaofei-pc
 * timeType_ 1、日账单 2、周账单 3、月账单
 */
public class FrmCommunityDayBill extends AbstractForm {

	@Override
	public IPage execute() throws Exception {
		JspPage page = new JspPage(this, "troubleshooting/commBill.jsp");
		LocalService getBill = new LocalService(this, "SvrGetCommBill");
        ...
        record.setField("type_",  this.getRequest().getParameter("type_"));
		if (!getBill.exec()) {
        ...
        page.add("type_", this.getRequest().getParameter("type_"));
        if("1".equals(this.getRequest().getParameter("type_")))
            page.add("title_", "社区归属日账单");
        if("2".equals(this.getRequest().getParameter("type_")))
            page.add("title_", "社区归属周账单");
        if("3".equals(this.getRequest().getParameter("type_")))
            page.add("title_", "社区归属月账单");
		return page;
	}
    ...
    public IPage excel() throws Exception {
        ...
        String type_ = getRequest().getParameter("type_");
	...
	public static void main(String[] args) {
		...
		Date d = FrmCommunityDayBill.paraseStringToDate("2014-8-1");
		List<String> lst3 = FrmCommunityDayBill.getAllweekDays(d);
		for (String s3 : lst3) {
			System.out.println(s3);
		}
		System.out.println("--------------根据特定的日期‘2014-8-1’获取特定日期所在一个月的每一天--------------");
		Date d2 = FrmCommunityDayBill.paraseStringToDate("2014-8-1");
		List<String> lst4 = FrmCommunityDayBill.getAllDaysMonthByDate(d2);
		for (String s3 : lst4) {
			System.out.println(s3);
		}
	}
