 ...
 *
 */
public class FrmOperateReported extends AbstractForm {

    @Override
    public IPage execute() throws Exception {
        JspPage page = new JspPage(this, "operate/FrmOperateReported.jsp");
        LocalService svr = new LocalService(this, "SvrGetOperateReported");
        if (!svr.exec())
            throw new RuntimeException(svr.getMessage());
        
        page.add("values", this.setPage(page, svr.getDataOut()));
		ParamUtil.addLog(this, "查询", "查询后台异常上报单列表", "查询后台异常上报单列表");
        return page;
    }
    ...
    public IPage getUser() throws Exception {
        ...
        record.setField("ywNo_", this.getRequest().getParameter("ywNo_"));
        record.setField("mobile_", this.getRequest().getParameter("mobile_"));
        record.setField("khmobile_", this.getRequest().getParameter("khmobile_"));
        record.setField("time_", this.getRequest().getParameter("time_"));
        record.setField("status_", this.getRequest().getParameter("status_"));
        if (!svr.exec())
            throw new RuntimeException(svr.getMessage());
        page.add("values", svr.getDataOut().getRecords());
        
        page.add("ywNo_", this.getRequest().getParameter("ywNo_"));
        page.add("mobile_", this.getRequest().getParameter("mobile_"));
        page.add("khmobile_", this.getRequest().getParameter("khmobile_"));
        page.add("time_", this.getRequest().getParameter("time_"));
        page.add("status_", this.getRequest().getParameter("status_"));
		ParamUtil.addLog(this, "查询", "模糊查询后台异常上报单列表", "模糊查询后台异常上报单列表");
        return page;
    }
     ...
     * @return
     */
    public JsonPage upstatus(){
    	String ywNo_ = this.getRequest().getParameter("ywNo_");
    	JsonPage json = new JsonPage(this);
    	LocalService upBinding = new LocalService(this, "SvrUpOperateStatus");
    	if(!upBinding.exec("ywNo_", ywNo_))
    		json.setData(false);
    	else
    		json.setData(true);
