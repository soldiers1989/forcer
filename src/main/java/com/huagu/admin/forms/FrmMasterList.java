...
/**
 * 
 * @ClassName: FrmMasterList
 * @Description: 后台师傅列表查询
 * @author li
 * @date 2017年12月11日
 *
 */
public class FrmMasterList extends AbstractForm {
	@Override
	public IPage execute() throws Exception {
		P004Browser page = new P004Browser(this);
		page.setTitle("师傅列表");// 设置标题
		...
		// 查询要显示的数据
		LocalService svr = new LocalService(this, "SvrMasterList");
		ParamUtil.ParamToHeadIn(page.getForm(), svr.getDataIn());
		if (!svr.exec())
			throw new RuntimeException(svr.getMessage());
		ParamUtil.addLog(this, "查询", "后台师傅列表查询", "后台师傅列表查询");
		
		DataSet ds = svr.getDataOut();
		// 根据自己想要的数据结构格式化数据
		if (ds.getRecNo() > 0) {
			for (Record r : ds.getRecords()) {
				r.getItems().put("zhuangtai", FrmMasterList.getState(r.getBoolean("enabled_"), r.getString("code_"), this));
			}
		}
		// 添加datagrid
	...
	public IPage updateState() throws Exception {
		...
		return new RedirectPage(this, "FrmMasterList");
	}
	...
	public static String getState(Boolean enabled_, String code_, IHandle handle) {
		if (enabled_ == false) {
			return "<input class='button addBtn'  type='button' onclick=\"javascript:window.location.href='FrmMasterList.updateState?state_=1&code_="
					+ code_ + "'\" value='禁用'/>";
		} else {
			return "<input class='button addBtn'  type='button' onclick=\"javascript:window.location.href='FrmMasterList.updateState?state_=0&code_="
					+ code_ + "'\" value='启用'/>";
		}
	}
	@Override
