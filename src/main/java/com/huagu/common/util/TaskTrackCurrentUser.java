...
public class TaskTrackCurrentUser extends AbstractTask {
	...
	public void execute() {
		...
		sql1.append(String.format("DELETE FROM %s WHERE DATEDIFF(NOW(),LoginTime_)>100", SystemTable.get(SystemTable.getCurrentUser)));
		conn.execute(sql1.toString());
		// 清除所有未正常登录的用户记录
		StringBuffer sql2 = new StringBuffer();
		sql2.append(String.format("UPDATE %s SET Viability_=-1 ", SystemTable.get(SystemTable.getCurrentUser)));
		// 在线达24小时以上的用户
